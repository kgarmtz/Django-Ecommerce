...
# Private Function: Fetching the Session ID from a cookie on the browser
...
def add_to_cart(request, product_id):
    ...
    if current_user.is_authenticated:
        # Checking if the product sent in the URL is already in the user Cart or Not
        is_cart_item_in_cart = CartItem.objects.filter(user=current_user,product=product).exists() 

        # Adding the products to the user Cart
        if is_cart_item_in_cart:
            # Getting all the user CartItems that correspond to the product sent in the URL  
            ...
            # Like "Blue and Large" exists in the existing_variations
            if product_variations in existing_variations:
                ...
                # Checking if the product_variations list is empty or not
        ...
        # If there's the first time that we add the product in the user Cart
        else:
...
# The user must be logged in the system to see this template
